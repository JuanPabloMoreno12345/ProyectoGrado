...
// --- Componente ligero que solo renderiza el To-Do board como acción cliente ---
class Mi90TodosAction extends Component {
  ...
  async load() {
    try {
      const data = await this.fetchJson('/mi90_eos/data/todos');
      const todos = data.todos || [];
      this.renderTodos(this.el, todos);
    } catch (e) {
      console.error('mi90_eos.todos load error', e);
    }
  }

  renderTodos(rootEl, todos) {
    // Simplemente reutilizamos la estructura de columnas que tenemos en la plantilla
    const board = rootEl.querySelector('.todos-board');
    if (!board) return;
    ...
    if (addBtn && addInput) {
      addBtn.onclick = async () => {
        ...
        try {
          ...
          addInput.value = '';
          await this.load();
        } catch(e) { console.error('todo create failed', e); } finally { addBtn.disabled = false; }
      };
    }

    rootEl.querySelectorAll('.mi90-todo-prev').forEach(btn => {
      btn.addEventListener('click', async (ev) => {
        const id = ev.currentTarget.dataset.id; await this._changeTodoStateByDelta(id, -1);
      });
    });
    rootEl.querySelectorAll('.mi90-todo-next').forEach(btn => {
      btn.addEventListener('click', async (ev) => {
    ...
    rootEl.querySelectorAll('.mi90-todo-delete').forEach(btn => {
      btn.addEventListener('click', async (ev) => {
        const id = ev.currentTarget.dataset.id;
        if (!confirm('¿Eliminar esta actividad?')) return;
        try {
          await this.fetchJson('/mi90_eos/data/todos/delete', {method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id: id})});
          await this.load();
        } catch(e) { console.error('todo delete failed', e); }
      });
    });
  }

  // --- Modal helpers for edit/create/delete ---
  _createModalElement() {
    // prevent creating multiple modals
    if (document.querySelector('.mi90-todo-modal-overlay')) return document.querySelector('.mi90-todo-modal-overlay');
    const overlay = document.createElement('div');
    overlay.className = 'mi90-todo-modal-overlay';
    ...
    box.className = 'mi90-todo-modal-box mi90--card';
    ...
    box.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        <input class="mi90-todo-modal-name" placeholder="Título" style="padding:8px;border:1px solid #e6e6e6;border-radius:6px" />
        <textarea class="mi90-todo-modal-desc" placeholder="Descripción" rows="4" style="padding:8px;border:1px solid #e6e6e6;border-radius:6px"></textarea>
        <div style="display:flex;gap:8px;align-items:center">
          <label style="font-size:12px;color:var(--muted);width:80px">Due:</label>
          <input type="date" class="mi90-todo-modal-due" />
          <label style="font-size:12px;color:var(--muted);margin-left:8px">Estado:</label>
          <select class="mi90-todo-modal-status">
        ...
        <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:6px">
          <button class="mi90-todo-modal-cancel mi90-todo-btn">Cancelar</button>
          <button class="mi90-todo-modal-delete mi90-todo-btn" style="background:#fee2e2">Eliminar</button>
          <button class="mi90-todo-modal-save mi90--nav-btn">Guardar</button>
        </div>
      </div>
    `;
  ...
  _closeModal() {
    const overlay = document.querySelector('.mi90-todo-modal-overlay');
  ...
  async _openEditModal(id) {
    ...
    try {
      ...
      const todos = res.todos || [];
      const t = todos.find(x => String(x.id) === String(id));
      const overlay = this._createModalElement();
      const nameEl = overlay.querySelector('.mi90-todo-modal-name');
      const descEl = overlay.querySelector('.mi90-todo-modal-desc');
      const dueEl = overlay.querySelector('.mi90-todo-modal-due');
      const statusEl = overlay.querySelector('.mi90-todo-modal-status');
      const saveBtn = overlay.querySelector('.mi90-todo-modal-save');
      const delBtn = overlay.querySelector('.mi90-todo-modal-delete');
      const cancelBtn = overlay.querySelector('.mi90-todo-modal-cancel');

      nameEl.value = t ? (t.name || '') : '';
      descEl.value = t ? (t.description || '') : '';
      dueEl.value = t && t.due_date ? t.due_date : '';
      statusEl.value = t ? (t.status || 'todo') : 'todo';

      cancelBtn.onclick = () => this._closeModal();
      saveBtn.onclick = async () => {
        saveBtn.disabled = true;
        try {
          const payload = { id: id, name: nameEl.value, description: descEl.value, due_date: dueEl.value, status: statusEl.value };
          await this.fetchJson('/mi90_eos/data/todos/update', {method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)});
          this._closeModal();
          await this.load();
        } catch(e) { console.error('todo save failed', e); } finally { saveBtn.disabled = false; }
      };
    ...
    } catch(e) { console.error('openEditModal failed', e); }
  }
  ...
  async _changeTodoStateByDelta(id, delta) {
    ...
    try {
      ...
      const cur = t.status || 'todo'; let idx = order.indexOf(cur); if (idx === -1) idx = 0;
      let newIdx = Math.min(order.length-1, Math.max(0, idx + delta));
      const newState = order[newIdx];
      await this.fetchJson('/mi90_eos/data/todos/update', {method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({id: id, status: newState})});
      await this.load();
    } catch(e) { console.error('change todo state failed', e); }
  }
}

registry.category("actions").add("mi90_eos.todo_board", Mi90TodosAction);
